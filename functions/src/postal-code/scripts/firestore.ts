...
import {CollectionReference, Query, WriteBatch, WriteResult}
...
export async function deleteCollection(
    collectionRef: CollectionReference, batchSize = 500) {
...
export async function execute(
    f: (batch: WriteBatch) => Promise<void>): Promise<WriteResult[]> {
...
export const registerAddresses = async () => {
  ...
  records.forEach((record: Address) => {
    ...
    const firstTwoDigis =
      record.postal_code ? record.postal_code.substring(0, 2) : ""
  ...
  //   (tp)=> tp.name.includes(cons.PUBSUB_TOPIC_ADD_DOC)
  // )?.[0]
